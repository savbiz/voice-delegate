"""Azure GA Realtime WebRTC with server-owned tools and bounded sideband control."""
...
class AzureRealtimeConnection(OpenAILiveConnection):
    """Reuse the bounded single-reader transport; translate only Azure commands."""

    normalize = staticmethod(normalize_event)

    def __init__(
        self, answer: WebRTCAnswer, socket: ClientConnection, provider: "AzureRealtimeProvider"
    ) -> None:
    ...
    async def connect(self, *, config: SessionConfig, offer_sdp: str) -> AzureRealtimeConnection:
        ...
        try:
            ...
            if not re.fullmatch(r"[A-Za-z0-9_-]{1,256}", candidate):
                raise ProviderError("Azure returned an invalid call identity; cleanup unconfirmed")
            call_id = candidate
            if not response.text.startswith("v=0") or len(response.content) > 65536:
                raise ProviderError("Azure returned an invalid SDP answer")
            ...
            return AzureRealtimeConnection(WebRTCAnswer(call_id, response.text), socket, self)
        except (
            httpx.HTTPError,
            ValidationError,
            WebSocketException,
            ...
            if isinstance(exc, asyncio.CancelledError):
                raise
            raise ProviderError("Azure connection failed; check server configuration") from exc
