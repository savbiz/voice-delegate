...
Consequences: public-provider events stay inside adapters. M1's `/token` capability returns 501; the supported Live browser setup is server-mediated SDP. Azure and Realtime adapters now share a WebRTC implementation with one explicit fallback attempt.
