...
test('voice preferences and accessible playback controls work before connecting', async ({ page }) => {
  await page.goto('/');
  await page.getByRole('button', { name: 'Live voice', exact: true }).click();
  await page.getByLabel('Response language').selectOption('it');
  await page.getByLabel('Conversation style').selectOption('mirror');
  await page.getByRole('button', { name: 'Mute assistant', exact: true }).click();
  await expect(page.getByRole('button', { name: 'Unmute assistant' })).toHaveAttribute('aria-pressed', 'true');
  await page.getByRole('button', { name: 'Large captions' }).click();
  await expect(page.getByRole('button', { name: 'Large captions' })).toHaveAttribute('aria-pressed', 'true');
