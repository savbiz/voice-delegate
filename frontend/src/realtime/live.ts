...
export function createLiveSession(
  store: LiveStore, audio: HTMLAudioElement, getAccessCode: () => string,
...
const onCancelTask = () => {
  ...
  if (owner) void request(`/sessions/${owner.id}/interrupt`, owner)
    .then(() => { if (session === owner && !ending) store.update({ workerMessage: "Worker cancellation requested. You can give a new request." }); })
    .catch(() => { if (session === owner && !ending) { store.update({ workerMessage: "Cancellation not confirmed. Try Cancel task again or stop the conversation.", cancelPending: false }); } });
};
...
async function recover(message: string): Promise<void> {
  ...
  finally { recovering = false; store.update({ recovering: false }); }
}
...
async function begin(existing?: Session, serverGeneration = 0): Promise<void> {
  ...
  try {
    ...
    if (attempt !== attemptId) { stream.getTracks().forEach(track => track.stop()); return; }
    microphone = stream;
    ...
    connection.addEventListener("track", event => {
      ...
      void audio.play().catch(() => { store.update({ status: "Press Play below to hear the assistant." }); });
    });
    connection.addEventListener("connectionstatechange", () => {
      if (attempt === attemptId && shouldRecover("connectionstatechange", connection.connectionState)) {
    ...
    channel.addEventListener("message", (message: MessageEvent<unknown>) => {
      if (attempt !== attemptId || typeof message.data !== "string" || message.data.length > 65536) return;
      try { receiveEvent(JSON.parse(message.data)); } catch { void finish("Invalid provider event."); }
    });
    channel.addEventListener("close", () => {
      if (attempt === attemptId && shouldRecover("data-channel-close")) void recover("Event connection closed.");
    });
...
const onFeedback = async (category: string) => {
  ...
  try {
    ...
    store.update({ feedbackStatus: `Report not confirmed. Check your access code or try later (maximum five reports per day). Retrying uses the same diagnostic ID: ${diagnosticId}.` });
  } finally {
    reportPending = false;
...
return {
  start: () => { if (!store.getSnapshot().active && !ending && !recovering) void begin(); },
  stop: () => { void finish('Conversation ended.'); },
  cancel: onCancelTask,
  feedback: onFeedback,
  dispose: () => { window.removeEventListener('pagehide', onPageHide); onPageHide(); },
};
}
