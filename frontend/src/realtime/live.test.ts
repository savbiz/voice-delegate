...
class FakeChannel extends EventTarget { close = vi.fn(); }
class FakePeer extends EventTarget {
  ...
  async setRemoteDescription() { this.emit({ type: 'session.created' }); }
  emit(event: unknown) { this.channel.dispatchEvent(new MessageEvent('message', { data: JSON.stringify(event) })); }
}
...
const source = { id: 's1', title: 'Architecture', section: 'History', path: 'docs/architecture.md', text: 'Evidence', digest: 'hash' };
let sources = [source];
const requests: { path: string; init: RequestInit }[] = [];

beforeEach(() => {
  vi.useFakeTimers();
  requests.length = 0; FakePeer.instances = []; responses = []; delegation = 'idle';
  closeResponse = undefined; sources = [source]; stopTrack = vi.fn();
  ...
  vi.stubGlobal('fetch', vi.fn(async (url: string, init: RequestInit) => {
    ...
    if (path.endsWith('/heartbeat')) {
      ...
      return Response.json({ state: 'connected', delegation, sources, generation: 0, fallback_available: true }, { status });
    }
...
async function start() {
  ...
  controller = createLiveSession(store, audio, () => 'invite', () => ({ language: 'auto', mode: 'conversation' }));
  controller.start();
...
test('heartbeat tolerates two failures, resets on success and finishes on the third consecutive failure', async () => {
  const store = await start(); responses = [500, 500, 200, 500, 500, 500];
...
test('local onset interrupts only after a running heartbeat', async () => {
  ...
  await vi.advanceTimersByTimeAsync(10000);
  onset?.();
  await vi.waitFor(() => expect(requests.filter(r => r.path.endsWith('/interrupt'))).toHaveLength(1));
...
test('stop releases media before close confirmation and stale confirmation cannot overwrite a new session', async () => {
  ...
  closeResponse = new Promise(resolve => { complete = resolve; });
  controller?.stop();
  expect(stopTrack).toHaveBeenCalledOnce();
...
test('same source ids retain display state and reconnect preserves captions and timings', async () => {
  ...
  peer.emit({ type: 'session.input_transcript.delta', delta: 'question', start_ms: 10, end_ms: 100 });
  peer.emit({ type: 'session.output_transcript.delta', delta: 'answer', start_ms: 150, end_ms: 200 });
