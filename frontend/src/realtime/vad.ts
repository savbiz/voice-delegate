...
export function watchSpeech(
  ...
  try {
    ...
    const timer = setInterval(() => {
      ...
      if (next !== remoteStream) {
        ...
        if (next?.getAudioTracks().length) {
          try {
            ...
            /* Provider-side speech detection still works without local analysis. */
          }
        }
      }
      const playing =
      ...
      const remoteRms =
        playing && remoteSource ? energy(remoteAnalyser, remoteSamples) * remoteAudio.volume : 0;
