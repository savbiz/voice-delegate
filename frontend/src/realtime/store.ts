...
export type LiveState = {
  ...
  captions: { user: string; assistant: string }; timings: TurnTiming[]; sources: Source[];
  recap: string; feedbackStatus: string; reportPending: boolean; reportSent: boolean; reportLocked: boolean;
};
...
export function initialLiveState(): LiveState {
  return {
    phase: 'ready', status: 'Ready to connect', help: 'Start a conversation when you are ready.',
    worker: 'idle', workerMessage: workerMessages.idle!, active: false, ending: false,
    recovering: false, cancelPending: false, captions: { user: '', assistant: '' }, timings: [],
    sources: [], recap: 'No conversation yet.', reportPending: false, reportSent: false, reportLocked: false,
...
export function workerState(state: LiveState, worker: string): LiveState {
  const phase = state.active && !state.ending && !state.recovering
    ? worker === 'running' ? 'working' : worker === 'busy' ? 'busy' : 'connected' : state.phase;
  return { ...state, phase, worker, cancelPending: false,
...
export function processEvent(state: LiveState, raw: unknown): {
  ...
  if (event.type === 'session.started' || event.type === 'session.created') {
    return { effect: 'started', state: { ...state, phase: 'connected',
  ...
  if (event.type === 'error') return { state: { ...state, status: 'The provider reported an error.',
  ...
  const speaker = ['session.input_transcript.delta', 'conversation.item.input_audio_transcription.completed'].includes(String(event.type))
