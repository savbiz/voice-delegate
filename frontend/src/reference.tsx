...
export function Reference({ code }: { code: string }) {
  ...
  async function search() {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL ?? ''}/api/reference/search`,
        {
          method: 'POST',
          headers: {
      ...
      if (!response.ok)
        throw new Error(
          response.status === 401
            ? 'Enter a valid personal invitation code.'
            : 'Documentation search is unavailable.',
        );
      ...
      setStatus(
        data.sources.length
          ? `${data.sources.length} source excerpts found. Open a source to inspect the evidence.`
          : 'No supporting documentation found. Try another project topic.',
      );
    } catch (error) {
  ...
  return (
    <section>
      ...
      <p className="notice">
        Read-only search · No microphone or paid model calls. Results quote a bundled documentation
        snapshot, not the live web or your server settings.
      </p>
      <form
        onSubmit={(event) => {
